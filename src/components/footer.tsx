...
// Link grows an underline from the left on hover (background-size trick)
...
export function Footer() {
  return (
    <footer className="relative border-t border-black/5">
      {/* Top accent line — primary fading to transparent */}
      <div
            ...
            <div className="mt-2 flex flex-col gap-1">
              {services.map((s) => (
                <Link
                  key={s}
                  href="/services"
                  className={`mt-1 block min-h-11 py-2 text-sm text-muted transition-colors hover:text-ink ${linkHover}`}
