    ...
    <nav className="sticky top-0 z-50 h-16 border-b border-black/5 bg-white/85 backdrop-blur-xl">
      <div className="mx-auto flex h-full max-w-6xl items-center justify-between px-4 sm:px-6 lg:px-8">
          ...
          {navLinks.map((item) => {
            ...
            return (
              <li key={item.href} className="relative">
                ...
                {active && <span className="absolute -bottom-[1.125rem] left-0 h-0.5 w-full rounded-full bg-primary" />}
              </li>
            );
          })}
        </ul>
        ...
        <div className="flex items-center gap-3">
          <a href={`tel:${phone}`} className="hidden items-center gap-2 whitespace-nowrap text-sm font-medium text-muted transition-colors hover:text-ink lg:flex">
            <Phone className="size-4 text-primary" /> {phone}
          </a>
          <Link href="/booking" className="hidden whitespace-nowrap rounded-full bg-primary px-5 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-primary-hover md:inline-block">
            จองบริการ
          </Link>
          <button type="button" onClick={() => setOpen((v) => !v)} aria-label={open ? "ปิดเมนู" : "เปิดเมนู"} aria-expanded={open} className="flex min-h-11 min-w-11 items-center justify-center rounded-lg p-2 text-ink transition-colors hover:bg-black/5 md:hidden">
      ...
      <div className={`overflow-hidden border-b border-black/5 bg-white md:hidden ${open ? "max-h-[80vh] opacity-100" : "max-h-0 opacity-0"}`} style={{ transition: "max-height 200ms ease, opacity 200ms ease" }}>
        <ul className="mx-auto max-w-6xl px-4 py-2 sm:px-6">
          ...
          <li>
            <a href={`tel:${phone}`} className="flex min-h-11 items-center gap-2 rounded-lg px-3 text-sm font-medium text-muted">
              <Phone className="size-4 text-primary" /> {phone}
            </a>
          </li>
          <li className="py-2">
            <Link href="/booking" onClick={() => setOpen(false)} className="flex min-h-11 items-center justify-center rounded-full bg-primary px-5 text-sm font-semibold text-white transition-colors hover:bg-primary-hover">
              จองบริการ
            </Link>
          </li>
        </ul>
