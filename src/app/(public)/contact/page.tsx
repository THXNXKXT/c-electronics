...
// ponytail: fallbacks — DB settings override these when present
const DEFAULTS = {
  phone: "0XX-XXX-XXXX",
  line: "@celectronics",
  email: "contact@celectronics.com",
  address: "เชียงราย",
  mondayFriday: "8:00 - 18:00",
  saturday: "8:00 - 17:00",
  sunday: "9:00 - 16:00",
};
...
export default async function ContactPage() {
  ...
  const line = s?.line || DEFAULTS.line;
  const email = s?.email || DEFAULTS.email;
  const address = s?.address || DEFAULTS.address;
  const mapsEmbed = s?.mapsEmbed || "https://maps.google.com/maps?q=Chiang%20Rai&t=&z=12&ie=UTF8&iwloc=&output=embed";

  const contactItems = [
    { icon: MapPin, label: "ที่อยู่", value: address },
    { icon: Phone, label: "โทรศัพท์", value: phone, href: `tel:${phone}` },
    { icon: MessageCircle, label: "LINE", value: line, href: LINE_URL },
    { icon: Mail, label: "อีเมล", value: email, href: `mailto:${email}` },
  ];

  const hours = [
    { day: "จันทร์ - ศุกร์", time: s?.mondayFriday || DEFAULTS.mondayFriday },
    { day: "เสาร์", time: s?.saturday || DEFAULTS.saturday },
    { day: "อาทิตย์", time: s?.sunday || DEFAULTS.sunday },
  ];
    ...
    <>
      <section className="border-b border-black/5 bg-canvas-muted">
        <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6 sm:py-14 lg:px-8">
          ...
          <a href={`tel:${phone}`} className="mt-6 inline-flex items-center gap-2 rounded-full bg-primary px-8 py-4 text-base font-bold text-white shadow-lg shadow-primary/20 transition-all hover:bg-primary-hover hover:shadow-xl">
        ...
        <div className="grid items-stretch gap-8 lg:grid-cols-2">
          <div className="flex flex-col">
            ...
            <div className="mt-5 overflow-hidden rounded-[16px] border border-black/5">
              {hours.map((h, i) => (
                <div key={h.day} className={`flex items-center justify-between px-5 py-3.5 ${i % 2 === 0 ? "bg-white" : "bg-canvas-muted"}`}>
          ...
          <div className="flex flex-col">
            ...
            <div className="mt-5 flex flex-1 flex-col overflow-hidden rounded-[20px] border border-black/5">
              <iframe src={mapsEmbed} width="100%" className="min-h-[400px] flex-1 lg:min-h-[600px]" style={{ border: 0 }} loading="lazy" referrerPolicy="no-referrer-when-downgrade" sandbox="allow-scripts allow-same-popups" title="แผนที่ C.Electronics เชียงราย" />
              <a href={MAPS_DIR_URL} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 bg-primary px-6 py-4 text-sm font-bold text-white transition-colors hover:bg-primary-hover">
