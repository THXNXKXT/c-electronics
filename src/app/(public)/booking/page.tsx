...
const trustBadges = [
  { icon: ShieldCheck, label: "ฟรีประเมิน" },
  { icon: Clock4, label: "ตอบกลับ 24 ชม." },
  { icon: Wrench, label: "ช่างมืออาชีพ" },
];

function genRef() {
  return "CE-" + Math.random().toString(36).slice(2, 8).toUpperCase();
}
    ...
    return (
      <div className="mx-auto flex max-w-2xl flex-col items-center px-4 py-24 text-center">
        ...
        <div className="mt-5 flex flex-wrap justify-center gap-3">
          ...
          >
            <PhoneCall className="size-4" /> โทรเลย
          </a>
          <button
            onClick={() => setSubmitted(false)}
            className="rounded-full border border-black/10 bg-white px-6 py-3 text-sm font-semibold text-ink transition-colors hover:border-ink"
          >
            ส่งคำขอใหม่
          </button>
        </div>
      ...
      <section className="mx-auto max-w-2xl px-4 py-12 sm:px-6 lg:px-8">
        ...
        <div className="mb-6 flex flex-wrap justify-center gap-3">
          {trustBadges.map((b) => {
            const Icon = b.icon;
            return (
              <div key={b.label} className="flex items-center gap-2 rounded-full border border-primary/15 bg-primary-tint px-4 py-2">
                <Icon className="size-4 text-primary" strokeWidth={2} />
                <span className="text-xs font-bold text-primary sm:text-sm">{b.label}</span>
              </div>
            );
          })}
        </div>

        {/* Reassuring header / progress */}
        <div className="mb-6 flex items-center justify-center gap-2 text-sm font-semibold text-muted">
          ...
          <span>ขั้นตอนที่ 1/1 — กรอกข้อมูล</span>
        </div>
        ...
        >
          ...
          <div>
            <h2 className="mb-4 flex items-center gap-2 text-sm font-bold uppercase tracking-wider text-subtle">
              <Wrench className="size-4 text-primary" /> รายละเอียดงาน
            </h2>
            <div className="space-y-5 border-l-2 border-primary/20 pl-5">
              <div>
                ...
                <select
                  name="serviceType"
                  required
                  onChange={(e) => setValid((v) => ({ ...v, serviceType: !!e.target.value }))}
          ...
          <div>
            <h2 className="mb-4 flex items-center gap-2 text-sm font-bold uppercase tracking-wider text-subtle">
              <User className="size-4 text-primary" /> ข้อมูลส่วนตัว
            </h2>
            <div className="space-y-5 border-l-2 border-primary/20 pl-5">
              <div className="grid gap-5 sm:grid-cols-2">
                <div>
                  ...
                  <div className="relative">
                    <User className="pointer-events-none absolute left-3.5 top-1/2 size-4 -translate-y-1/2 text-subtle" />
                    <input
                      name="name"
                      type="text"
                      required
                      placeholder="ชื่อของคุณ"
                      onChange={(e) => setValid((v) => ({ ...v, name: e.target.value.trim().length >= 2 }))}
                      className="w-full rounded-xl border border-black/10 bg-white py-2.5 pl-10 pr-9 text-sm outline-none transition-colors focus:border-primary"
                    />
                ...
                <div>
                  ...
                  <div className="relative">
                    <Phone className="pointer-events-none absolute left-3.5 top-1/2 size-4 -translate-y-1/2 text-subtle" />
                    <input
                      name="phone"
                      type="tel"
                      required
                      placeholder="08X-XXX-XXXX"
                      onChange={(e) => setValid((v) => ({ ...v, phone: checkPhone(e.target.value) }))}
                      className="w-full rounded-xl border border-black/10 bg-white py-2.5 pl-10 pr-9 text-sm outline-none transition-colors focus:border-primary"
                    />
              ...
              <div className="grid gap-5 sm:grid-cols-2">
                <div>
                  ...
                  <select
                    name="district"
                    required
                    onChange={(e) => setValid((v) => ({ ...v, district: !!e.target.value }))}
                    className="w-full rounded-xl border border-black/10 bg-white px-4 py-2.5 text-sm outline-none transition-colors focus:border-primary"
                  >
                ...
                <div>
                  ...
                  <div className="relative">
                    <Calendar className="pointer-events-none absolute left-3.5 top-1/2 size-4 -translate-y-1/2 text-subtle" />
              ...
              <div>
                ...
                <div className="relative">
                  <Home className="pointer-events-none absolute left-3.5 top-1/2 size-4 -translate-y-1/2 text-subtle" />
          ...
          <button
            type="submit"
            disabled={submitting}
            className="flex w-full items-center justify-center gap-2 whitespace-nowrap rounded-full bg-primary px-6 py-3.5 text-sm font-semibold text-white transition-colors hover:bg-primary-hover disabled:cursor-not-allowed disabled:opacity-70"
          >
            {submitting ? (
